exports = module.exports = function(userConfig) {
  ...
  var nodeConfig = new nodeFileConfig("modernmt-translate-server");
  ...
  var modernmt = require("modernmt-translate")({key: config.key});
  ...
  var app = {
    ...
    start: function() {
      ...
      express.get("/languages", [authorization], async function(request, response) {
        var {languages} = await app.wrapper("languages", modernmt.languages());
      ...
      express.post("/translate", [authorization, empty("text"), empty("from"), empty("to")], async function(request, response) {
        ...
        if (typeof result !== "undefined") {
          ...
          var {translation} = await app.wrapper("translation", modernmt.translate(text, from, to));
          if (typeof translation !== "undefined") {
            ...
            if (typeof result !== "undefined") {
              ...
              console.log("Could not save to database: ", from, to, text, translation);
              response.send(JSON.stringify({status: app.status.success, message: translation, extra: "Could not save to database. Please check server."}));
            }
          } else {
